exports.handler = async (event) => {
  ...
  async function getActivePlayers(espnTeamId) {
    ...
    try {
      ...
      // Fetch box scores for last 5 games and collect players who appeared
      ...
      for (const box of boxScores) {
        ...
        for (const team of box.boxscore.players) {
          // Only include players from the correct team
          ...
          for (const stat of (team.statistics || [])) {
            for (const athlete of (stat.athletes || [])) {
              ...
              const mins = athlete.stats?.[0] || "0";
              // Only include if they actually played (minutes > 0)
              if (name && mins !== "0" && mins !== "DNP" && mins !== "--") {
