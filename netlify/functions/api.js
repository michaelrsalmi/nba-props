exports.handler = async (event) => {
  ...
  function buildScheduleResponse(events, h) {
    ...
    return { statusCode:200, headers:h, body: JSON.stringify({games}) };
  }

  // ── GET: NBA schedule ─────────────────────────────────────────────────
  if (event.httpMethod === "GET" && !event.queryStringParameters?.type) {
    const today = todayET();
    const d = await get(`https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=${today}`);
    if (!d?.events || !d.events.length) {
      // fallback to default scoreboard
      const d2 = await get("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard");
      if (!d2?.events) return { statusCode:500, headers, body: JSON.stringify({error:"Schedule failed"}) };
      const games = d2.events.filter(e => {
        const status = e.competitions?.[0]?.status?.type?.description || '';
        return !['Final','Forfeit','STATUS_FINAL'].includes(status);
      });
      if (games.length) {
        return buildScheduleResponse(games, headers);
      }
      return { statusCode:500, headers, body: JSON.stringify({error:"No games today"}) };
    }
    return buildScheduleResponse(d.events, headers);
  }
  ...
  if (event.httpMethod === "GET" && event.queryStringParameters?.type === "sgp-of-day") {
    ...
    const result = parseAIJson(await aiCall(
      ...
      `Today: ${today}. Games: ${gameList}\n\nFANDUEL ALT LINES (-150 to +400):\n${fdStr}\n\nBuild the BEST SGP across any games using LOCK lines (odds -200 to -2000).
Stack enough legs so the COMBINED parlay odds land between +150 and +400.
Typically 5-6 legs at -300 to -600 each hits this range.
Only use lines tagged as 'lock' from FD data. State the estimated combined odds.\n\nReturn ONLY:\n{"title":"SGP OF THE DAY","subtitle":"...","legs":[{"player":"Name","team":"ABV","game":"AWAY @ HOME","prop":"OVER 22.5 PTS (+105)","reason":"..."}],"totalLegs":4,"estimatedOdds":"+380","correlation":"...","confidence":4}`,
      1000
    ));
...
3. For SGP legs: use lock lines (odds -200 to -2000) — high confidence legs
4. Stack 4-6 lock legs so the COMBINED parlay odds land between +150 and +400
5. To estimate combined odds: multiply implied probs together. e.g. 4 legs at -400 each = 80%×80%×80%×80% = 41% implied = roughly +144. Add more legs to get into +150-+400 range.
6. Aim for 5-6 legs at -300 to -600 each which typically produces +200 to +350 combined
7. For each leg: calculate impliedProb from fdOdds, estimate modelProb, edge = modelProb - impliedProb
8. B2B teams: -1 confidence
9. suggestedSGP must state the estimated combined parlay odds (target +150 to +400)
...
Include 8 legs. Mix all 4 categories.`;
  ...
  // ── POST: Save props for tracking ───────────────────────────────────
  if (event.httpMethod === "POST" && event.queryStringParameters?.type === "save-props") {
    try {
      const { getStore } = require("@netlify/blobs");
      const body = JSON.parse(event.body || "{}");
      const store = getStore("prop-results");
      const date = todayET();
      const key = `props:${date}:${body.gameId}`;
      ...
      // Merge new legs
      const merged = [...existing, ...(body.legs || [])];
      await store.set(key, JSON.stringify(merged));
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, saved: merged.length }) };
    } catch(e) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: e.message }) };
    }
  }

  // ── GET: Check results for a date ────────────────────────────────────
  if (event.httpMethod === "GET" && event.queryStringParameters?.type === "check-results") {
    try {
      const { getStore } = require("@netlify/blobs");
      const date = event.queryStringParameters?.date || todayET();
      const store = getStore("prop-results");

      // Get all saved props for this date
      const keys = await store.list({ prefix: `props:${date}:` });
      if (!keys?.blobs?.length) return { statusCode: 200, headers, body: JSON.stringify({ results: [], date }) };

      const allProps = [];
      for (const blob of keys.blobs) {
        const raw = await store.get(blob.key, { type: "text" });
        if (raw) allProps.push(...JSON.parse(raw));
      }

      // Get game scores from ESPN for this date
      const scores = await get(`https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=${date}`);

      // Check each prop against box scores
      const results = await Promise.all(allProps.map(async (prop) => {
        try {
          // Find the game
          const game = (scores?.events || []).find(e => {
            const comp = e.competitions?.[0];
            ...
            return home === prop.homeAbbr || away === prop.awayAbbr;
          });
          if (!game) return { ...prop, result: "pending" };

          const status = game.competitions?.[0]?.status?.type?.description || "";
          if (!["Final", "STATUS_FINAL"].includes(status)) return { ...prop, result: "pending" };

          // Get box score
          const box = await get(`https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=${game.id}`);
          if (!box?.boxscore?.players) return { ...prop, result: "no_data" };
          ...
          for (const team of box.boxscore.players) {
            for (const stat of (team.statistics || [])) {
              for (const ath of (stat.athletes || [])) {
                ...
                const propName = (prop.player || "").toLowerCase();
                if (name.includes(propName.split(" ").pop()) || propName.includes(name.split(" ").pop())) {
                  const s = ath.stats || [];
                  // ESPN order: min, fg, 3pt, ft, oreb, reb, ast, stl, blk, to, pf, pts, +/-
                  playerStats = {
                    pts: parseFloat(s[11]) || 0,
                    reb: parseFloat(s[5]) || 0,
                    ast: parseFloat(s[6]) || 0,
                    fg3m: parseFloat((s[2]||"0-0").split("-")[0]) || 0,
                  };
                  break;
                }
              }
          ...
          if (!playerStats) return { ...prop, result: "not_found" };

          // Parse the prop threshold
          const propStr = (prop.prop || "").toLowerCase();
          let threshold = parseFloat(propStr.match(/(\d+\.?\d*)/)?.[1] || 0);
          let actual = 0;
          let statName = "";

          if (propStr.includes("pts") || propStr.includes("point")) { actual = playerStats.pts; statName = "PTS"; }
          else if (propStr.includes("reb")) { actual = playerStats.reb; statName = "REB"; }
          else if (propStr.includes("ast")) { actual = playerStats.ast; statName = "AST"; }
          else if (propStr.includes("three") || propStr.includes("3pm") || propStr.includes("threes")) { actual = playerStats.fg3m; statName = "3PM"; }

          const hit = actual >= threshold;
          return { ...prop, result: hit ? "hit" : "miss", actual, statName, threshold };
        } catch(e) {
          return { ...prop, result: "error", error: e.message };
        }
      }));

      // Save results back
      for (const blob of keys.blobs) {
        const gameId = blob.key.split(":")[2];
        const gameResults = results.filter(r => r.gameId === gameId);
        if (gameResults.length) await store.set(blob.key, JSON.stringify(gameResults));
      }

      return { statusCode: 200, headers, body: JSON.stringify({ results, date }) };
    } catch(e) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: e.message }) };
    }
  }
  ...
  // ── GET: All-time record ──────────────────────────────────────────────
  if (event.httpMethod === "GET" && event.queryStringParameters?.type === "record") {
    try {
      const { getStore } = require("@netlify/blobs");
      const store = getStore("prop-results");
      const allKeys = await store.list({ prefix: "props:" });
      if (!allKeys?.blobs?.length) return { statusCode: 200, headers, body: JSON.stringify({ record: {}, days: [] }) };

      const record = { total: 0, hits: 0, misses: 0, byCategory: {}, byConfidence: {}, recentDays: [] };
      const dayMap = {};

      for (const blob of allKeys.blobs) {
        const raw = await store.get(blob.key, { type: "text" });
        if (!raw) continue;
        const props = JSON.parse(raw);
        const date = blob.key.split(":")[1];

        if (!dayMap[date]) dayMap[date] = { date, hits: 0, misses: 0, props: [] };

        for (const p of props) {
          if (!["hit","miss"].includes(p.result)) continue;
          const isHit = p.result === "hit";
          record.total++;
          if (isHit) record.hits++; else record.misses++;

          const cat = p.category || "OTHER";
          if (!record.byCategory[cat]) record.byCategory[cat] = { hits:0, total:0 };
          record.byCategory[cat].total++;
          if (isHit) record.byCategory[cat].hits++;

          const conf = String(p.confidence || 3);
          if (!record.byConfidence[conf]) record.byConfidence[conf] = { hits:0, total:0 };
          record.byConfidence[conf].total++;
          if (isHit) record.byConfidence[conf].hits++;

          if (isHit) dayMap[date].hits++; else dayMap[date].misses++;
          dayMap[date].props.push({ player: p.player, prop: p.prop, result: p.result, actual: p.actual, threshold: p.threshold, statName: p.statName });
        }
      }

      record.hitRate = record.total > 0 ? Math.round(record.hits / record.total * 100) : 0;
      record.recentDays = Object.values(dayMap).sort((a,b) => b.date.localeCompare(a.date)).slice(0, 14);
