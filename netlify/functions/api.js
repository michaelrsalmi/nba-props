  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      if (!game || !game.homeTeam) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: "No game data", body: event.body }) };
      }
...
Use your knowledge of the 2025-26 NBA season. Return ONLY a raw JSON object, nothing else:
      ...
      try {
        ...
        if (match) {
          try {
            ...
            return { statusCode: 500, headers, body: JSON.stringify({ error: "JSON parse failed", raw: raw.substring(0, 300), aiStatus: aiRes.status }) };
          }
        } else {
          return { statusCode: 500, headers, body: JSON.stringify({ error: "No JSON found in response", raw: raw.substring(0, 300) }) };
        }
      }
