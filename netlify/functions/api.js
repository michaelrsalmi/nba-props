exports.handler = async (event) => {
    ...
    try {
      ...
      for (const market of (fd.markets || [])) {
        for (const outcome of (market.outcomes || [])) {
          ...
          const cat = market.key;

          if (!player || name !== "Over") continue;
          // Only include odds between -150 and +400 (sweet spot for SGP alt lines)
          if (price < -150 || price > 400) continue;
  ...
  // ── SGP OF THE DAY — fetches best SGP across all games ──────────────
  if (event.httpMethod === "GET" && event.queryStringParameters?.type === "sgp-of-day") {
    ...
    const games = espn.events.map(e => {
      const comp = e.competitions?.[0];
      const home = comp?.competitors?.find(c => c.homeAway === "home");
      const away = comp?.competitors?.find(c => c.homeAway === "away");
      return `${away?.team?.displayName} @ ${home?.team?.displayName} (${new Date(e.date).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",timeZone:"America/New_York"})})`;
    }).join("\n");

    // Get FD alt lines across all games
    const allFDLines = {};
    for (const e of espn.events.slice(0,4)) {
      ...
      const away = comp?.competitors?.find(c => c.homeAway === "away");
      if (home && away) {
        const lines = await getFanDuelAltLines(home.team.displayName, away.team.displayName);
        Object.assign(allFDLines, lines);
      }
    }

    const fdStr = Object.keys(allFDLines).length
      ? Object.entries(allFDLines).map(([p, cats]) =>
          `${p}: ` + Object.entries(cats).map(([cat, lines]) =>
            cat.replace("player_","").replace("_alternate","").toUpperCase() + ": " +
            lines.map(l => `${l.line}(${l.odds>0?"+"+l.odds:l.odds})`).join(",")
          ).join(" | ")
        ).join("\n")
      : "No FD lines available";

    const today = new Date().toDateString();
    const prompt = `You are a sharp NBA SGP analyst. Today: ${today}.

TODAY'S NBA GAMES:
${games}

FANDUEL ALT LINES AVAILABLE (odds between -150 and +400):
${fdStr}

Build the BEST 4-5 leg SGP of the day across ANY games. 
Pick correlated legs that maximize value.
Only use players/lines from the FD data above.

Return ONLY raw JSON:
{
  "title": "🐝 SGP OF THE DAY",
  "subtitle": "Best correlated parlay across tonight's slate",
  "legs": [
    { "player": "Full Name", "team": "ABBR", "game": "AWAY @ HOME", "prop": "OVER 22.5 PTS (+105)", "reason": "one sentence" }
  ],
  "totalLegs": 4,
  "estimatedOdds": "+380",
  "correlation": "why these legs work together",
  "confidence": 4
}`;
    ...
    const ai = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model:"claude-haiku-4-5-20251001", max_tokens:1000,
        system:"Sharp NBA SGP analyst. Return ONLY valid JSON. No markdown.",
  ...
  // ── SINGLE BEST PROPS — top individual props across all games ─────────
  if (event.httpMethod === "GET" && event.queryStringParameters?.type === "best-props") {
    ...
    const today = new Date().toDateString();
    const prompt = `You are a sharp NBA prop analyst. Today: ${today}.
Games: ${games}

FANDUEL LINES (odds -150 to +400):
${fdStr}

Find the 8 BEST single game props from tonight's entire slate.
Focus on value — good odds, high hit rate, clear edge.
Only use lines from FD data above.
  ...
  "props": [
    {
      ...
      "team": "ABBR", 
      "game": "AWAY @ HOME",
      "prop": "OVER 22.5 PTS (+105)",
      "category": "POINTS",
      "last10": 8,
      "last5": 4,
      "confidence": 4,
      "edge": "one sentence sharp take"
    }
  ]
}`;
    ...
    const ai = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model:"claude-haiku-4-5-20251001", max_tokens:1500,
        system:"Sharp NBA prop analyst. Return ONLY valid JSON. No markdown.",
        messages:[{role:"user",content:prompt}] }),
    });
    const aiData2 = await ai.json();
    let raw2 = (aiData2.content||[]).map(b=>b.text||"").join("").trim();
    raw2 = raw2.replace(/^```json\s*/i,"").replace(/^```\s*/i,"").replace(/\s*```$/i,"").trim();
    try {
      return { statusCode:200, headers, body: raw2 };
    } catch {
      return { statusCode:500, headers, body: JSON.stringify({ error:"Parse failed" }) };
    }
  }
...
Return ONLY raw JSON:
{
  ...
  "keyFactors": ["factor 1", "factor 2", "factor 3"]
      ...
      raw = raw.replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```$/i, "").trim();
      let parsedResult;
      try { parsedResult = JSON.parse(raw); }
      catch {
        const match = raw.match(/\{[\s\S]*\}/);
        if (match) {
          try { parsedResult = JSON.parse(match[0]); }
          catch { return { statusCode: 500, headers, body: JSON.stringify({ error: "Parse failed", raw: raw.substring(0, 200) }) }; }
        } else {
          return { statusCode: 500, headers, body: JSON.stringify({ error: "No JSON found", raw: raw.substring(0, 200) }) };
        }
      }
      return { statusCode: 200, headers, body: JSON.stringify(parsedResult) };
    }
  ...
  "sgpLegs": [
    {
      ...
      "edge": 18,
      "confidence": 4,
      "usageBoost": "+6% without Morant or empty string",
      "injuryNote": "",
      "reason": "one sentence sharp take"
    }
  ],
