exports.handler = async (event) => {
  ...
  async function fetchInjuries(homeAbv, awayAbv, gameId) {
    ...
    // Fetch both the general injury list AND game-specific injury report
    const [listData, gameData] = await Promise.all([
      safeFetch(`https://tank01-fantasy-stats.p.rapidapi.com/getNBAInjuryList`, { headers: TANK_HEADERS }),
      gameId ? safeFetch(`https://tank01-fantasy-stats.p.rapidapi.com/getNBAGameInfo?gameID=${gameId}`, { headers: TANK_HEADERS }) : Promise.resolve(null),
    ]);

    const allInjuries = [];

    // From general injury list
    if (listData?.body) {
      const injuries = Array.isArray(listData.body) ? listData.body : Object.values(listData.body);
      ...
      for (const p of filtered) {
        ...
        allInjuries.push({ name, team, status, desc, key: name.toLowerCase() });
      }
    }

    // From game-specific data (often more accurate/up to date)
    if (gameData?.body?.teamInjuries || gameData?.body?.injuries) {
      const gameInj = gameData.body.teamInjuries || gameData.body.injuries || [];
      const injList = Array.isArray(gameInj) ? gameInj : Object.values(gameInj);
      for (const p of injList) {
        const name = p.playerName || p.longName || p.name || "Unknown";
        const team = p.teamAbv || p.team || "";
        const status = p.injStatus || p.status || p.designation || "Out";
        const desc = p.injDescription || p.description || "";
        const key = name.toLowerCase();
        // Only add if not already in list
        if (!allInjuries.find(x => x.key === key)) {
          allInjuries.push({ name, team, status, desc, key });
        }
      }
    }

    return {
      lines: allInjuries.length
        ? allInjuries.map(p => `${p.name} (${p.team}) — ${p.status}${p.desc ? ": "+p.desc : ""}`)
  ...
  if (event.httpMethod === "POST") {
    ...
    const [homeData, awayData, injuryData, b2b] = await Promise.all([
      ...
      fetchInjuries(game.homeAbbr, game.awayAbbr, game.id),
