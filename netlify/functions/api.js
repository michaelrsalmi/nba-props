exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      if (!OPENAI_KEY) {
        return { statusCode: 500, headers, body: JSON.stringify({ error: "Missing OPENAI_API_KEY env var" }) };
      }
...
Use your knowledge of the 2024-25 NBA season. Return ONLY a raw JSON object, nothing else:
  ...
  "props": [
    {
      ...
      "avg": "27.3",
      "confidence": 4,
      "reason": "one sentence with specific stat context"
    }
  ]
}

Include 6-8 props covering both teams across all 4 categories.`;
      ...
      const aiRes = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a JSON API. Your entire response must be a single valid JSON object. No markdown, no backticks, no text before or after the JSON."
            },
      ...
      if (aiData.error) {
        return { statusCode: 500, headers, body: JSON.stringify({ error: "OpenAI error: " + aiData.error.message }) };
      }
      ...
      try {
        ...
        if (match) {
          ...
          return { statusCode: 500, headers, body: JSON.stringify({ error: "No JSON in response", raw: raw.substring(0, 300) }) };
        }
      }
