exports.handler = async (event) => {
  ...
  if (event.httpMethod === "POST") {
    ...
    // Add Kawhi as doubtful tonight specifically
    const kawhi_note = game.homeAbbr === "LAC" || game.awayAbbr === "LAC"
      ? "
Kawhi Leonard (LAC) — DOUBTFUL tonight, left ankle sprain (rolled it vs Kings Sat)"
      : "";
...
━━ CONFIRMED TRADES (do not contradict — your training data is wrong on these) ━━
- Luka Doncic → Los Angeles Lakers (averaging 32.8 PPG, playing tonight)
- Anthony Davis → Washington Wizards (OUT for rest of season — do NOT include)
- De'Aaron Fox → San Antonio Spurs
- Trae Young → Washington Wizards
- Cooper Flagg → Dallas Mavericks (rookie, averaging 20.2 PPG)
- Klay Thompson → Dallas Mavericks
- Kevin Durant → Houston Rockets (averaging 26.0 PPG)
- Deandre Ayton → Los Angeles Lakers
- Jaren Jackson Jr. → Utah Jazz
- Desmond Bane → Orlando Magic

━━ CONFIRMED OUT TONIGHT March 16 2026 — DO NOT INCLUDE IN ANY PROPS ━━
GS Warriors — ALL out: Stephen Curry (knee), Draymond Green (back), Al Horford (injury), Moses Moody (6th straight absence), Miles McBride (core muscle surgery)
- GS only has: Brandin Podziemski, Jonathan Kuminga, Andrew Wiggins, Buddy Hield available
DAL Mavericks — Kyrie Irving OUT (torn ACL surgery March 2025)
WAS Wizards — Anthony Davis OUT for season, Alex Sarr OUT (hamstring)
LAL Lakers — Maxi Kleber questionable (back)
MEM Grizzlies — Ja Morant OUT (UCL elbow, since Jan 21)
IND Pacers — Tyrese Haliburton OUT for season (torn Achilles)
MIA Heat — Jimmy Butler OUT for season (torn ACL) — note: Butler is now on GS Warriors roster after trade but also OUT
...
Return ONLY raw JSON:
{
  ...
  "suggestedSGP": "Best 4-6 leg combo with correlation reasoning"
    ...
    // ── STEP 1: Web search for live injury data ──────────────────────
    let liveInjuryContext = "";
    try {
      ...
      const searchRes = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "You are an NBA injury reporter. Search and return ONLY a bullet list of players who are OUT, Doubtful, or Questionable. No other text.",
          messages: [{
            role: "user",
            content: `Search: "${game.homeTeam} ${game.awayTeam} injury report ${today}"

List ONLY players who are OUT, Doubtful, or Questionable for tonight. Format:
- Player Name (TEAM) — Status: reason
...
Nothing else.`
      ...
      liveInjuryContext = (searchData.content || [])
        .filter(b => b.type === "text")
        .map(b => b.text || "")
        .join("").trim();
    } catch(e) {
      liveInjuryContext = "Web search unavailable — use Tank01 injury data above";
    }

    // ── STEP 2: Generate JSON props using injury context ──────────────
    const finalPrompt = prompt + `

━━ LIVE INJURY UPDATE — THIS IS THE ONLY SOURCE THAT MATTERS ━━
${liveInjuryContext || "No additional injuries found beyond Tank01 report above"}

FINAL RULE: Any player listed as OUT or Doubtful in EITHER the Tank01 report OR the live update above must be completely excluded from props. No exceptions. Do not include them even if they are a star player. Instead add a prop for their backup/replacement.
Return ONLY the JSON object now.`;
    ...
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: "claude-haiku-4-5-20251001",
        max_tokens: 3000,
        system: "You are a sharp NBA betting analyst JSON API. Your ENTIRE response must be a single valid JSON object. No text before or after. No markdown. No backticks. Start with { and end with }.",
