    ...
    try {
      const searchRes = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "You are an NBA injury researcher. Search for injury info and return ONLY a plain text summary of who is OUT or QUESTIONABLE. No JSON, no formatting, just bullet points.",
          messages: [{
            role: "user",
            content: `Search for today's injury report: "${game.homeTeam} ${game.awayTeam} injury report ${new Date().toDateString()}". List every player who is OUT or Questionable for tonight's game with their status. Plain text only.`
          }],
        }),
...
━━ LIVE INJURY UPDATE (web search just ran) ━━
${liveInjuryContext}

IMPORTANT: The live injury update above is the most current data. 
Any player listed as OUT or Doubtful above must be excluded from all props.
